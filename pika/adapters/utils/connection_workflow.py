...
class AMQPConnectionWorkflowFailed(AMQPConnectorException):
    ...
    def __repr__(self):
        return (f'{self.__class__.__name__}: {len(self.exceptions)} exceptions in all; last exception - {self.exceptions[-1]!r}; first '
...
class AMQPConnector:
    ...
    def _on_tcp_connection_timeout(self):
        ...
        error = AMQPConnectorSocketConnectError(
            socket.timeout(f'TCP connection attempt timed out: {self._conn_params.host!r}/{self._addr_record}'))
    ...
    def _on_overall_timeout(self):
        ...
        if prev_state == self._STATE_AMQP:
            msg = (f'Timeout while setting up AMQP to {self._conn_params.host!r}/{self._addr_record}; ssl={bool(self._conn_params.ssl_options)}')
            _LOG.error(msg)
            # Initiate close of AMQP connection and wait for asynchronous
